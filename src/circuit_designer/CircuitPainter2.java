...
public class CircuitPainter2 {
			...
			CompoundCircuit c1)
	{
		...
		for (int i = 0; i < c1.columns.size(); i++) // gate drawing phase
		{
			...
			for (int j = 0; j < c1.columns.get(i).size(); j++)
			{
				...
				painter.drawString("y"+selected_gate.getIndex(), (int)(x_cntr+(icon_width*0.3)), (int)(y_cntr+(icon_height*0.6)));
			}
		}
	}
	...
	public static BufferedImage drawCircuit(CompoundCircuit c1) {
		...
		//drawConnection(image, 20, 50, 200, 80, Color.RED);
		for (int i = 0; i < c1.columns.size(); i++)
		{
			for (int j = 0; j < c1.columns.get(i).size(); j++)
			{
